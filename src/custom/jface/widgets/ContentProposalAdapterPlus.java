 ...
 * {@link org.eclipse.jface.fieldassist.ContentProposalAdapter} can be used to
 * attach content proposal behavior to a control. This behavior includes
 * obtaining proposals, opening a popup dialog, managing the content of the
 * control relative to the selections in the popup, and optionally opening up a
 * secondary popup to further describe proposals.
 * <p>
 * The main difference with the original class is that it provides a way for
 * having a user-defined table in matters of appearance and behavior for hosting
 * the proposals by providing a table provider {@link IProposalTableProvider}
 * instance to this class using method
 * {@link #setProposalTableProvider(IProposalTableProvider)
 * setContentProposalProvider}. Moreover, methods for enabling a single click
 * selection {@link #setSingleClickSelection(boolean) setSingleClickSelection}
 * of proposals and setting the proposals pop-up location
 * {@link #setPopupLocation(int) setPopupLocation} are also provided.
 * </p>
 * <p>
 ...
 * @since 3.2
 */
public class ContentProposalAdapterPlus {
	...
	class ContentProposalPopup extends PopupDialog {
			...
			if (singleClickSelection) {
				proposalTable.addMouseListener(new MouseAdapter() {
					public void mouseUp(MouseEvent e) {
						...
						if (proposalTable.getSelectionCount() < 1)
							return;
		...
		private void resizeProposalsTable() {
			...
			Shell sh = proposalTable.getShell();
			if (sh == null) {
				return;
			}

			Rectangle area = sh.getClientArea();
			...
			if (size.y > area.height + proposalTable.getHeaderHeight()) {
			...
			TableColumn[] tbl_cols = proposalTable.getColumns();
			tbl_cols[0].pack();

			if (tbl_cols.length > 1) {
				tbl_cols[1].pack();

				if (tbl_cols[0].getWidth() + tbl_cols[1].getWidth() < width) {
					tbl_cols[1].setWidth(width - tbl_cols[0].getWidth());
				}
			} else {
				tbl_cols[0].setWidth(width);
			}

			proposalTable.setSize(area.width, area.height);
		...
		protected void adjustBounds() {
			...
			if (popupLocation == POPUP_LEFT) {
				Point controlSize = control.getSize();
				int controlWidth = controlSize.x;
				offset = popupSize.x - controlWidth + POPUP_OFFSET;
			}
	 ...
	 * The object that provides content proposals.
	 */
	private IProposalTableProvider tableProvider;
	...
	private void notifyPopupOpened() {
		...
		for (int i = 0; i < listenerArray.length; i++) {
			...
			// listenerArray[i]).proposalPopupOpened(this);
		}
	}
	...
	private void notifyPopupClosed() {
		...
		for (int i = 0; i < listenerArray.length; i++) {
			// ((IContentProposalListener2)
			// listenerArray[i]).proposalPopupClosed(this);
		}
	}
