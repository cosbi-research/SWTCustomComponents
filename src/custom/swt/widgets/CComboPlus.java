...
/**
 * The CComboPlus is a modified version of {@link org.eclipse.swt.custom.CCombo}
 * class and represents a selectable user interface object that combines a text
 * field and a list and issues notification when an item is selected from the
 * list.
 * <p>
 * CComboPlus provides new methods for having item selection on mouse hover
 * {@link #setMouseHoverSelection(boolean) setMouseHoverSelection} which is
 * enabled by default and also, for changing the text color of the editable text
 * {@link #setTextForeground(Color) setTextForeground}
 * </p>
 * <p>
 ...
 *      information</a>
 */
public class CComboPlus extends Composite {
	 ...
	 * @see Widget#getStyle()
	 */
	public CComboPlus(Composite parent, int style) {
		...
		listener = new Listener() {
			public void handleEvent(Event event) {
				...
				if (CComboPlus.this == event.widget) {
					comboEvent(event);
					return;
				}
		...
		filter = new Listener() {
			public void handleEvent(Event event) {
				...
				if (shell == CComboPlus.this.getShell()) {
					handleFocus(SWT.FocusOut);
				}
			}
		};
	...
	void listEvent(Event event) {
		...
		case SWT.MouseMove: {
			...
			int index = list.getTopIndex() + p.y / height;
	...
	/**
	 * Enables the selection of list items on mouse hover if argument is
	 * <code>true</code>, otherwise it disables it.
	 * 
	 * @param enable
	 ...
	 * 
	 */
	public void setMouseHoverSelection(boolean enable) {
		if (enable)
			list.addListener(SWT.MouseMove, listener);
		else
			list.removeListener(SWT.MouseMove, listener);

	}

	/**
	 * Sets the CCombo's text foreground color to the one specified by the
	 * argument
	 * 
	 * @param color
	 *            - the new color
	...
	/**
	 * Clear the combo-box list, if it's present
	 */
