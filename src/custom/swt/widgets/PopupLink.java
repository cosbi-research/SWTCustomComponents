...
/**
 * The PopupLink class represents an interactive user interface object that has
 * the look of link, but combines a text field and/or a dropdown list (depending
 * on its type) in a popup dialog for changing its content.
 * <p>
 * This custom widget can be used in user input forms or any other place that an
...
public class PopupLink extends Composite {
	...
	private List<ChangeSelectionLister> changeSelectionListeners = new ArrayList<ChangeSelectionLister>();
	private List<OpenDialogListener> openDialogListeners = new ArrayList<OpenDialogListener>();
	private List<CloseDialogListener> closeDialogListeners = new ArrayList<CloseDialogListener>();
	...
	private void installListeners() {
		...
		link.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				...
				for (OpenDialogListener openDialogListener : openDialogListeners) {
					openDialogListener.onOpenDialog();
				}
	...
	/**
	 * @param value
	 *            - the text to be displayed in the text field of the popup link
	 */
	...
	/**
	 * @param options
	 *            - the dropdown list options of the popup link
	 */
	...
	/**
	 * @param selItemIndex
	 *            - the index of the option to appear as selected
	 */
	...
	/**
	 * @param connection
	 *            - a string to used as connection between the textfield value
	 *            and the selected option e.g. 10 per sec
	 */
	...
	/**
	 * Updated the text displayed by the link taking which is the combindation
	 * of the text field value (if any) and the selected option from the
	...
	/**
	 * @return the list of {@link IPopupLinkInputValidator} objects that are
	 *         used for validating the user input
	 */
	...
	/**
	 * @param validators
	 *            - a list of {@link IPopupLinkInputValidator} objects to be
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.swt.widgets.Control#setCursor(org.eclipse.swt.graphics.
	 * Cursor)
	 */
	@Override
	...
	private class OurLayout extends Layout {
		...
		protected Point computeSize(Composite composite, int wHint, int hHint, boolean changed) {
			...
			if (changed || tExtent == null) {
				tExtent = children[0].computeSize(SWT.DEFAULT, SWT.DEFAULT, false);
			}
	...
	public void addOpenDialogListener(OpenDialogListener listener) {
		this.openDialogListeners.add(listener);
	}

	public void addCloseDialogListener(CloseDialogListener listener) {
		this.closeDialogListeners.add(listener);
		PopupControlAdapter.getInstance().addClosedialogListener(listener);
	}
	...
	public static abstract class OpenDialogListener {
		public abstract void onOpenDialog();
	}

	public static abstract class CloseDialogListener {
		public abstract void onCloseDailog();
	}
}
