...
import custom.swt.widgets.PopupLink.CloseDialogListener;
...
public class PopupControlAdapter {
	...
	public void closePopupControl() {
		...
		for (CloseDialogListener closeDialogListener : closeDialogListeners) {
			closeDialogListener.onCloseDailog();
		}
	}
	...
	public class PopupControl extends PopupDialog {
		...
		public final class PopupCloserListener implements Listener {
			...
			public void handleEvent(final Event e) {
				...
				// If the focus switching between controls on the popup we jsu
				// ignore those FocusOut events.
				// Insted when the focus is leaving the Popup shell, we then
				// close the popup/
				 ...
				 */
					...
					if (activeShell == selPopupLink.getShell()) {
						...
						// Force close when focus is transferred in a window
			...
			void installListeners() {
				// Listeners on this popup's controls (text, combo and scroll
				// bar)
			...
			void removeListeners() {
				if (isValid()) {
					// Listeners on this popup's controls (text, combo and
					// scroll bar)
					for (Control c : popupContent.getChildren()) {
				...
				public void modifyText(ModifyEvent e) {
					...
					if (columns > 0) {
						...
						if (comboCtrl != null) {
							new_width = width + POPUP_SIZE_THRESHOLD + text.getBorderWidth() * 2
									+ comboCtrl.getBounds().width;
						} else {
							new_width = width + POPUP_SIZE_THRESHOLD + text.getBorderWidth() * 2;
						}

						getShell().setBounds(getShell().getBounds().x, getShell().getBounds().y, new_width,
					...
					if (selPopupLink.getValidators() != null) {
						...
						for (IPopupLinkInputValidator v : selPopupLink.getValidators()) {
							...
							if (popupType == PopupControlAdapter.TYPE_TEXT_DROPDOWN)
								message = v.validateInput(value, selPopupLink.getDropdownOptions(),
										comboCtrl.getSelectionIndex());
							else
				...
				comboCtrl.addSelectionListener(new SelectionListener() {
					...
					public void widgetSelected(SelectionEvent e) {
						...
						if (popupType == PopupControlAdapter.TYPE_DROPDOWN) {
							...
							// Validate text input using the defined validators
							// if any
							if (selPopupLink.getValidators() != null) {
